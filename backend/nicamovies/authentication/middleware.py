...
class AuthenticatedUser():
    ...
    def __call__(self, request):
        if self.is_protected_route(request):
            try:
                token = request.headers.get('Authorization')[7:] if request.headers.get('Authorization') else json.loads(request.body).get('Authorization')[7:]
    ...
    def is_protected_route(self, request):
        return (
            ...
            resolve(request.path_info).url_name in ['create_rating', 'users', 'user']
        )
